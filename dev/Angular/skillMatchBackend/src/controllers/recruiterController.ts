...
import { Recruiter, Job, Candidate, Metric } from '../utils/types/index';
...
export const getRecruiterMetrics = async (req: Request, res: Response) => {
  try {
    ...
    const applicationRateResult = await pool.query(applicationRateQuery, [recruiterId]);
    ...
    const qualityResult = await pool.query(qualityQuery, [recruiterId]);
    ...
    const timeToHireResult = await pool.query(timeToHireQuery, [recruiterId]);
...
export const getActiveJobs = async (req: Request, res: Response) => {
  try {
    ...
    const jobsResult = await pool.query(jobsQuery, [recruiterId]);
...
export const getCandidateMatches = async (req: Request, res: Response) => {
  try {
    ...
    const recruiterJobsResult = await pool.query(recruiterJobsQuery, [recruiterId]);
    ...
    const matchesResult = await pool.query(matchesQuery, [jobIds]);
...
export const contactCandidate = async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(
      insertMessageQuery, 
      [sender_id, receiver_id, related_job_id, content]
    );
